...
- **Node-level statistics**: Collects per-node metrics for CPU, memory, JVM, file system, transport, and thread pools.
...
Monitor per-node resource usage (CPU, heap, RAM), JVM garbage collection behaviour, thread pool saturation, and file system utilisation to identify bottlenecks before they impact production workloads.
